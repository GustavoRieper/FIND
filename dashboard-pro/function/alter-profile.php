<?php
    ...
    $sql= mysqli_query($connect, "SELECT name, last_name, senha, cep, rua, bairro, cidade, uf FROM professional WHERE '$email'= email") or die (mysql_error());
    ...
    if($cep!=$dados['cep']){
        $update = mysqli_query($connect, "UPDATE professional set cep='$cep' where '$email'= email");
        echo("CEP Alterado<br>");
    }
    if($rua!=$dados['rua']){
        $update = mysqli_query($connect, "UPDATE professional set rua='$rua' where '$email'= email");
        echo("Rua Alterado<br>");
    }
    if($bairro!=$dados['bairro']){
        $update = mysqli_query($connect, "UPDATE professional set bairro='$bairro' where '$email'= email");
        echo("Bairro Alterado<br>");
    }
    if($cidade!=$dados['cidade']){
        $update = mysqli_query($connect, "UPDATE professional set cidade='$cidade' where '$email'= email");
        echo("Cidade Alterado<br>");
    }
    if($uf!=$dados['uf']){
        $update = mysqli_query($connect, "UPDATE professional set uf='$uf' where '$email'= email");
        echo("UF Alterado<br>");
    }
